...
from config import OUTPUT_DIR, ARCHIVE_PASSWORD, TEMPLATE_DOCX, ASSIGN_TO_ID, RESOLVED_STATUS_ID, PROJECT_PASSWORDS, REPORT_CONFIG
...
def run(ticket_ids=None):
    ...
    for ticket in tickets:
        ...
        subprocess.run([sys.executable, mkdocx_py, '--template', TEMPLATE_DOCX, '--image', A_img, '--out', docx_out], check=True)
        ...
        if project_key is not None:
            if str(project_key) in proj_pw:
                ...
                try:
                    create_report_issue(report_project, subject, description, assigned_to_id=report_assignee)
